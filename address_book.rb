...
def main_menu
  if Contact.all.length < 1
    ...
    if main_choice == 'n'
      ...
      list_contacts
      main_menu
    elsif main_choice =~ /^[-+]?[0-9]+$/
      number_choice = main_choice.to_i
...
def add_email(contact)
  ...
  puts "#{new_email.address} has been added to #{contact.full_name}\n"
  email_menu(contact)
end
...
def add_address(contact)
  ...
  puts "#{new_address.address} has been added to #{contact.full_name}\n"
  address_menu(contact)
end
